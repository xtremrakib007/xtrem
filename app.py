...
# Database setup
def init_database():
    conn = sqlite3.connect('multi_role_system.db')
    c = conn.cursor()
    ...
    if not c.fetchone():
        admin_password = 'Rakib009'  # In a real app, you should hash the password
        c.execute('''
            INSERT INTO users (username, first_name, last_name, email, phone, id_number, password, role, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', ('xtremrakib', 'MD RAKIBUL', 'ISLAM', 'rakib@example.com', '0123456789', 'A1234567', admin_password, 'admin', 'approved'))
...
def get_db_connection():
    return sqlite3.connect('multi_role_system.db')
